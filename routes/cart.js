...
router.get('/:userId', async (req, res) => {
  const userId = req.params.userId;                         
    try {   
    const cart = await req.dbclient.db(DB_NAME).collection(COLLECTION_NAME).findOne({ userId: new ObjectId(userId) });
    if (!cart) {
...
router.post('/add', async (req, res) => {
  ...
  const itemExists = await req.dbclient.db(DB_NAME).collection(COLLECTION_NAME).findOne(query);

  if (itemExists) {

const currentProduct = itemExists.items.find(item => item.productId.equals(productId));
// SAFETY CHECK: If current quantity is 1 and user sends -1, REMOVE the item
    if (currentProduct.quantity <= 1 && quantity === -1) {
      ...
      return res.send({ success: true, message: "Item removed from cart!", result });
    }
    ...
    await req.dbclient.db(DB_NAME).collection(COLLECTION_NAME).updateOne(query, updateDoc);
  } else {
   //New Items will added from here if there is no item in card with same product of this user.
    ...
    await req.dbclient.db(DB_NAME).collection(COLLECTION_NAME).updateOne(filter, updateDoc, { upsert: true });
  }
